# Round 01 - Instance Layer and Frozen Config (v2)
...
- `reference/legacy/m1_data_setup.py`
- `data/runtime_12/parameters.json`
- `data/runtime_12/normal_load_scenarios.csv`
- `data/runtime_12/normal_ev_demand_slow.csv`
- `data/runtime_12/normal_ev_demand_fast.csv`
- `data/runtime_12/disaster_load_scenarios.csv`
- `data/runtime_12/disaster_ev_discharge_slow.csv`
- `data/runtime_12/disaster_ev_discharge_fast.csv`
...
3. Implement an adapter that reads the runtime source-of-truth layers using this precedence:
   - schema/loader behavior from `reference/legacy/m1_data_setup.py`
   - runtime numeric params from `data/runtime_12/parameters.json`
   - actual scenario support from live CSV contents in `data/runtime_12/`
4. Treat `reference/legacy/m1_data_setup.py` as a **reference-only contract source**.
   - Do not inherit generator/demo defaults such as `A_SCEN = 10` or `B_SCEN = 10` as live runtime truth.
   - Do not make mainline runtime loading depend on executing generator code from that file.
5. Implement explicit validation for the default runtime fixture contract:
...
6. Build topology helpers for the IEEE 33-bus radial network:
   - root identification via frozen config,
   - parent/child maps,
   - downstream-line lookup,
   - radiality checks.
7. Implement stable integer/string index maps for buses, lines, regions, normal times, disaster times, scenarios.
8. Implement explicit fail-fast validation rules, including at least:
...
9. The schema must preserve the fact that `critical_buses` is a required external paper-level input for the disaster objective.
...
10. Provide clear error messages designed for debugging, not vague assertions.
