...
def test_experiment_pack_smoke_produces_core_outputs(tmp_path: Path) -> None:
    ...
    assert (output_root / "plans" / "integrated_mainline_paper_like_smoke_plan.csv").exists()
    assert (output_root / "plans" / "normal_only_paper_like_smoke_plan.csv").exists()
    assert (output_root / "logs" / "integrated_mainline_paper_like_smoke_run.json").exists()
    assert (output_root / "logs" / "integrated_mainline_paper_like_smoke_iteration_log.json").exists()
    assert (output_root / "logs" / "normal_only_paper_like_smoke_run.json").exists()
