...
\end{aligned}
\end{equation}
\hspace*{0.7em}If the optimal value of \eqref{eq:sg_sep_milp_main} is at most $\varepsilon_{\mathrm{cert}}$, the current RMP solution is certified. Otherwise, let $\delta^{\star}$ be an optimal outage vector of \eqref{eq:sg_sep_milp_main}. The sample-wise dual LPs
\[
\max_{\pi_b\in\Pi}\ \pi_b^\top(H^b-Z^b x^{\star}+U\delta^{\star}),
\qquad b=1,\dots,B,
\]
are then re-solved and extreme optimal dual solutions $\widehat{\pi}_b^{\star}\in\mathrm{ext}(\Pi)$ are selected. The new cut coefficients are obtained by setting $\boldsymbol{\pi}=(\widehat{\pi}_1^{\star},\dots,\widehat{\pi}_B^{\star})$, and the corresponding cut is added to $\mathcal{R}$.
...
at $(x^k,\alpha^k,\lambda^k)$, and obtain its optimal value
$\widehat{v}^k$ and an optimal outage vector $\delta^k$.
\STATE \hspace{1em} If $\widehat{v}^k \le \varepsilon_{\mathrm{cert}}$,
